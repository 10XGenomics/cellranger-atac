...
class KMedoids:
    ...
    def fit_predict(self, matrix):
        ...
        for t in xrange(self.tmax):
            ...
            good_medoid_label = []
            for medoid_label in range(self.nmedoids):
                # This one-liner can be memory intensive on a large bloc
                # J = np.mean(distance_matrix[np.ix_(clusters[medoid_label], clusters[medoid_label])], axis=1)
                J = np.zeros(len(clusters[medoid_label]))
                for i, pt in enumerate(clusters[medoid_label]):
                    for pt2 in clusters[medoid_label]:
                        J[i] += distance_matrix[pt, pt2]
                if J.size > 0:
                    j = np.argmin(J)
            ...
            if self.force and len(bad_medoid_label) > 0:
