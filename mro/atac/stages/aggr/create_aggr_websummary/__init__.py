...
def get_aggr_metrics_data(metadata, summary_data, library_list):
    ...
    metrics = metadata.gen_metric_list(summary_data, metric_keys, species_list=library_list, species_is_library=True)
...
def get_clustering_plots(analysis, filtered_peak_bc_matrix, species_list, library_list, singlecell_df, is_barnyard):
    data = {}
    ...
    data['aggr_cluster_plot'] = generate_clustering_plot(singlecell_df, tsne_results, cluster_labels, barcodes,
                                                         species_list, library_list=library_list, method="library")
    ...
    if is_barnyard:
        helptext[0][1] += ["(bottom right) Scatter plot of barcodes annotated as cells, colored by species."]
...
def main(args, outs):
    ...
    add_data(websummary_data, get_aggr_metrics_data(metadata, summary_data, library_list))

    add_data(websummary_data, get_clustering_plots(args.analysis, args.filtered_peak_bc_matrix, species_list, library_list,
