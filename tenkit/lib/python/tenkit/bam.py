...
def make_pg_header(version, stage_name, product_name='longranger'):
    ''' Generate a BAM PG header line for the stage that is manipulating the bam file '''
    pg = {
        'ID': stage_name,
        'PN': '{0}.{1}'.format(product_name, stage_name),
        'VN': version
    }
    return pg

def make_terminal_pg_header(version, product_name='longranger'):
    ''' Generate a PG header line that denotes the entire longranger pipeline, for tracking purposes.'''
    pg = {'ID': product_name, 'PN': product_name, 'VN': version}
    return pg
...
def create_bam_outfile(file_name, chrom_names, chrom_lengths, template=None, pgs=None, cos=None, rgs=None, replace_rg=False):
    ...
    if template:
        ...
        if rgs is not None:
            if replace_rg and 'RG' in header and len(rgs) > 0:
                header['RG'] = []
            for rg in rgs:
                if not 'RG' in header:
                    header['RG'] = []
